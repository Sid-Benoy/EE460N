...
int GetLD_TEMP_STATE(int* x) { return x[LD_TEMP_STATE];}
int GetOR0(int* x)           { return x[OR0]; }
int GetOR1(int* x)           { return x[OR1]; }
/***************************************************************/
...
#define PTE_PROTECTED_MASK 0x0008
#define PTBR_MASK 0xFF00
#define VA_MASK 0xFE00
/***************************************************************/
/* A cycle counter.                                            */
/***************************************************************/
int CYCLE_COUNT;
...
void eval_micro_sequencer() {
    ...
    if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION) == 1){
    ...
    else if(CURRENT_LATCHES.STATE_NUMBER == 61){
        int next_state = lookup(CURRENT_LATCHES.TEMP_STATE);
        ...
        if(CURRENT_LATCHES.EXC){
            ...
            if(CURRENT_LATCHES.STATE_NUMBER == 60){
                if(CURRENT_LATCHES.TEMP_STATE == 7 || CURRENT_LATCHES.TEMP_STATE == 3)                  //lab 5 functionality for VA -> PA translation
                    j_2 = 1;
            }
            ...
            if(CURRENT_LATCHES.STATE_NUMBER == 18 && CURRENT_LATCHES.INT){
                next_state = 49;
            }
            memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[next_state], sizeof(int) * CONTROL_STORE_BITS);
            NEXT_LATCHES.STATE_NUMBER = next_state;
...
void drive_bus() {
    ...
    if(GetGATE_PTBR(CURRENT_LATCHES.MICROINSTRUCTION))
        BUS = Low16bits(result);
...
void latch_datapath_values() {
    ...
    if(CURRENT_LATCHES.STATE_NUMBER == 10 || CURRENT_LATCHES.STATE_NUMBER == 11) {                  //illegal opcode exception
        NEXT_LATCHES.EXC = 1;
        NEXT_LATCHES.EXCV = (0x5 << 1) + 0x0200;
    }
    ...
    if(GetLD_VA(CURRENT_LATCHES.MICROINSTRUCTION)){
        NEXT_LATCHES.VA = CURRENT_LATCHES.MAR;                          //state 57
    }

    if(GetGATE_PTBR(CURRENT_LATCHES.MICROINSTRUCTION)){
        int MAR = ((CURRENT_LATCHES.PTBR & PTBR_MASK)) + (((CURRENT_LATCHES.VA & VA_MASK) >> 9) << 1);              //state 58, get virtual address
        if(MAR % 2 != 0) {
            NEXT_LATCHES.EXC = 1;
            NEXT_LATCHES.EXCV = (0x3 << 1) + 0x0200;
        }
        else
            NEXT_LATCHES.MAR = MAR;
    }

    if(CURRENT_LATCHES.STATE_NUMBER == 56){                                                     //check for exception
        if(privilege == 1 && !(CURRENT_LATCHES.MDR & PTE_PROTECTED_MASK)){
            NEXT_LATCHES.EXC = 1;
            NEXT_LATCHES.EXCV = (0x4 << 1) + 0x0200;
        }
        else if(!(CURRENT_LATCHES.MDR & PTE_VALID_MASK)){
            NEXT_LATCHES.EXC = 1;
            NEXT_LATCHES.EXCV = (0x2 << 1) + 0x0200;
        }
    }

    if(GetOR0(CURRENT_LATCHES.MICROINSTRUCTION)){                                       //state 60
        NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR | 0x0001;
    }

    if(GetOR1(CURRENT_LATCHES.MICROINSTRUCTION)){                                       //state 63
        NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR | 0x0002;
    }

    if(GetGATE_VA(CURRENT_LATCHES.MICROINSTRUCTION)){                                                                       //state 62
    ...
    if(GetLD_PC(CURRENT_LATCHES.MICROINSTRUCTION)){
        ...
        NEXT_LATCHES.TEMP_STATE = CURRENT_LATCHES.STATE_NUMBER;
